...
class TestParseFlexibleDate:
    ...
    def test_invalid_date_format(self):
        ...
        with pytest.raises(ValueError, match="Date '2023/01/15' must be in YYYY-MM-DD or YYYY-M-D format"):
...
class TestCLIMain:
    ...
    def test_pressure_levels_parsing(self, mock_logger, mock_era5ify_geojson):
        ...
        test_args = [
            ...
            '--geojson', 'test.geojson',
            '--dataset-type', 'pressure',
            '--pressure-levels', '1000, 925, 850, 500'  # With spaces
        ]
    ...
    def test_variable_parsing_with_spaces(self, mock_logger, mock_era5ify_geojson):
        ...
        test_args = [
            'geojson',
            '--request-id', 'test-123',
            '--variables', 'temperature, humidity, wind_speed',  # With spaces
            '--start', '2023-01-01',
            '--end', '2023-01-31',
            '--geojson', 'test.geojson'
        ]
...
# Parametrized tests
@pytest.mark.parametrize("mode,extra_args", [
