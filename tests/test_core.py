...
def test_load_and_validate_geojson():
    ...
    with patch('varunayan.core.load_json_with_encoding', return_value=mock_geojson), \
        ...
        mock_logger.info.assert_called_with("\x1b[0;32m✓ GeoJSON loaded successfully\x1b[0m")
...
def test_validate_inputs_with_geojson(mock_exists):
...
@patch('shutil.rmtree')
@patch('glob.glob')
def test_cleanup_temp_files(mock_glob, mock_rmtree, mock_remove, mock_exists):
...
def test_process_era5_data_pressure_level_success(mock_open_dataset, mock_download, mock_extract, mock_filter, mock_logger, pressure_params):
    ...
    mock_ds = xr.Dataset({
        't': (['valid_time', 'pressure_level', 'latitude', 'longitude'], np.random.rand(24, 2, 2, 2)),
        'u': (['valid_time', 'pressure_level', 'latitude', 'longitude'], np.random.rand(24, 2, 2, 2))
    }, coords={
        'valid_time': pd.date_range('2020-01-01', periods=24, freq='h'),
