...
def check_cdsapi_config() -> bool:
    ...
    try:
        with open(cds_file, 'r') as f:
            ...
            if len(lines) >= 2:
                # Multi-line format
                url_line = next((line for line in lines if line.strip().startswith('url:')), None)
                key_line = next((line for line in lines if line.strip().startswith('key:')), None)
...
def _setup_notebook_widget():
    ...
    def on_clipboard_clicked(b):
        with output:
            ...
            if clipboard_key:
                ...
                print("Please copy your API key and try again, or type it manually in the text area above.")
    ...
    display(widgets.VBox([
        api_key_widget,
        widgets.HBox([submit_button, clipboard_button]),
        output
    ]))
...
def setup_cdsapi_config():
    ...
    print("You need a CDS account and API key from: https://cds.climate.copernicus.eu/profile")
        ...
        if completion_state is None:
            ...
            def check_setup_completion():
                if completion_state['completed']:
                    ...
                    print("⏳ Setup not completed yet. Please fill in your API key and click Submit above.")
                    return False
