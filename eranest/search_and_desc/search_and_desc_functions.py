...
def describe_variables(variable_names, dataset_type):
    """
    ...
    if len(matches) > 0:
        ...
        for category, vars_in_category in category_groups.items():
            ...
            for i, var in enumerate(vars_in_category, 1):
                ...
                dataset_info = f" (from {var['dataset']} levels)" if dataset_type == "all" else ""
...
def _process_single_dataset(dataset_type):
...
def _process_pressure_dataset(dataset_type):
