...
from ..exceptions import AuroraError, DataProcessingError, ValidationError
...
try:
    ...
    logger.warning(
        "PyTorch not available - Aurora batch creation will use mock tensors"
    )
...
class AuroraBatchInfo:
    ...
    def __post_init__(self):
        ...
        if self.time_steps < 2:
            raise AuroraError(
                "Aurora requires at least 2 time steps (current + previous)"
            )
...
class AuroraConverter:
    ...
    def __init__(
        ...
        if not TORCH_AVAILABLE:
            logger.warning(
                "PyTorch not available - using numpy arrays instead of tensors"
            )
    ...
    def _analyze_data_structure(
        ...
        unique_lats = sorted(
            coords_df[lat_coord].unique(), reverse=True
        )  # Aurora wants decreasing
        ...
        if atmospheric_data is not None and pressure_coord in atmospheric_data.columns:
            pressure_levels = sorted(
                atmospheric_data[pressure_coord].unique(), reverse=True
            )
        ...
        if atmospheric_data is not None:
            atmospheric_vars = [
                col
                for col in atmospheric_data.columns
                if col not in [lat_coord, lon_coord, time_coord, pressure_coord]
            ]

        if static_data is not None:
            static_vars = [
                col for col in static_data.columns if col not in [lat_coord, lon_coord]
            ]
    ...
    def _convert_surface_variables(
        self,
    ...
    def _convert_atmospheric_variables(
        self,
        ...
        for era5_var in batch_info.atmospheric_variables:
            if era5_var in self.atmospheric_mapping:
                ...
                for t_idx, time_val in enumerate(selected_times):
                    if time_val is not None:
                        time_data = atmospheric_data[
                            atmospheric_data[time_coord] == time_val
                        ]
                    else:
                        time_data = atmospheric_data
                    ...
                    for _, row in time_data.iterrows():
                        ...
                        if pd.notna(value):
                            if TORCH_AVAILABLE:
                                tensor[0, t_idx, pressure_idx, lat_idx, lon_idx] = (
                                    float(value)
                                )
                            else:
                                tensor[0, t_idx, pressure_idx, lat_idx, lon_idx] = (
                ...
                logger.warning(
                    f"No Aurora mapping for atmospheric variable: {era5_var}"
                )

        return atmos_vars

    def _convert_static_variables(
        self,
    ...
    def _create_metadata(self, batch_info: AuroraBatchInfo) -> Dict[str, Any]:
        ...
        if TORCH_AVAILABLE:
            ...
            lons = torch.linspace(0, 360, batch_info.width + 1, dtype=torch.float32)[
                :-1
            ]
        else:
...
def create_aurora_batch(
    ...
    **kwargs,
    ...
    """
    converter = AuroraConverter(**kwargs)
    aurora_data = converter.convert_era5_to_aurora(
