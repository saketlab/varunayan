...
from ..exceptions import GeospatialError, ValidationError

logger = logging.getLogger(__name__)


def validate_coordinates(
    latitude: float, longitude: float, strict: bool = True
) -> bool:
    """
...
def extract_bounding_box(
    geojson_data: Dict[str, Any], buffer: Optional[float] = None
...
def create_spatial_index(
    ...
    gdf = gpd.GeoDataFrame(
        {"id": range(len(geometries))}, geometry=geometries, crs="EPSG:4326"
    )
...
def buffer_geometry(
    geometry: Union[Polygon, MultiPolygon], distance: float, resolution: int = 16
) -> Union[Polygon, MultiPolygon]:
    """
