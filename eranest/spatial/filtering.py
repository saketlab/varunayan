...
from ..constants import SPATIAL_FILTER_CHUNK_SIZE, USE_SPATIAL_INDEX, DEFAULT_PARALLEL_WORKERS
from ..exceptions import GeospatialError, DataProcessingError
from .geometry import validate_coordinates, merge_geometries
...
class SpatialFilter:
    ...
    def filter_dataset(self,
                      ...
                      lon_coord: str = "longitude") -> FilterResult:
        """
        ...
        logger.info(f"Pre-filtered to {len(candidate_lats)} candidate points using bounding box")
        ...
        logger.info(f"Filtered {result.filtered_points}/{result.total_points} points ({result.inside_percentage:.1f}%)")
    ...
    def filter_dataframe(self,
                        ...
                        lon_col: str = "longitude") -> FilterResult:
        """
        ...
        if lat_col not in df.columns or lon_col not in df.columns:
            raise DataProcessingError(f"Required columns not found: {lat_col}, {lon_col}")
        
        total_points = len(df)
        
        # Pre-filter using bounding box
        bbox_filtered = self._bbox_filter_dataframe(df, shapely_geom.bounds, lat_col, lon_col)
        ...
        inside_mask = self._filter_points_by_geometry(
            bbox_filtered[lat_col].values,
            bbox_filtered[lon_col].values,
            shapely_geom
        )
    ...
    def _geojson_to_shapely(self, geojson: Dict[str, Any]) -> Union[Polygon, MultiPolygon]:
    ...
    def _bbox_filter_dataframe(self,
                              df: pd.DataFrame,
    ...
    def _filter_points_by_geometry(self,
                                  lats: np.ndarray,
                                  lons: np.ndarray,
                                  geometry: Union[Polygon, MultiPolygon]) -> np.ndarray:
        """
    ...
    def _filter_points_simple(self,
                             lats: np.ndarray,
                             lons: np.ndarray,
                             geometry: Union[Polygon, MultiPolygon]) -> np.ndarray:
    ...
    def _filter_points_chunked(self,
                              lats: np.ndarray,
                              lons: np.ndarray,
                              geometry: Union[Polygon, MultiPolygon]) -> np.ndarray:
    ...
    def _filter_points_parallel(self,
                               lats: np.ndarray,
                               lons: np.ndarray,
                               geometry: Union[Polygon, MultiPolygon]) -> np.ndarray:
    ...
    def _filter_with_spatial_index(self,
                                  points: List[Point],
                                  geometry: Union[Polygon, MultiPolygon]) -> np.ndarray:
    ...
    def _extract_data_for_coordinates(self,
                                    dataset: xr.Dataset,
                                    lats: np.ndarray,
                                    lons: np.ndarray,
                                    lat_coord: str,
                                    lon_coord: str) -> pd.DataFrame:
        ...
        # Filter DataFrame
        mask = df.apply(
            lambda row: (row[lat_coord], row[lon_coord]) in coord_pairs,
            axis=1
        )
...
def optimize_spatial_filtering(
    ...
    if isinstance(geometry, dict):
        ...
        gdf = gpd.GeoDataFrame.from_features([geometry] if geometry.get("type") == "Feature" else geometry["features"])
    ...
    if hasattr(simplified, 'geoms'):
        # MultiPolygon
        large_geoms = [geom for geom in simplified.geoms if geom.area > tolerance * tolerance]
        if large_geoms:
